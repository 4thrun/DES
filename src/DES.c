...
void En_DES(const bool M[64],bool K[64],char final[16]){
    ...
    CreateKeyOfLoop(K, Key);	// Generate key
    for(loop=0; loop<16; loop++){	// 16 loops
        for(i=0; i<32; i++) L2[i] = R1[i];
        F(R1, Key[loop], result);
        for(i=0; i<32; i++){
    ...
    for(i=0; i<16; i++){
        ...
        if(num>=0&&num<=9) final[i] = to_ascii('0'+num);
        else final[i] = to_ascii('A' + num - 10);
    }
}
...
void De_DES(const bool M[64], bool K[64], char final[16]){
    ...
    CreateKeyOfLoop(K, Key);
    for(loop=0; loop<16; loop++){	// 16 loops
        for(i=0; i<32; i++) L2[i] = R1[i];
        F(R1, Key[15-loop], result);
        for(i=0; i<32; i++){
            if(result[i]==L1[i]) R2[i] = 0;
            else R2[i] = 1;
        }
